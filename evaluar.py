...
data_dir = 'data'
eventos_gt = []
json_files = sorted([f for f in os.listdir(data_dir) if f.endswith('.json')], key=lambda f: int(f.split('.')[0]))
...
results_dir = r".\resultados\gemma-3-27b-it"
results_dir = r".\resultados\gemini-2.5-flash"

eventos_pred = []
json_files = sorted([f for f in os.listdir(results_dir) if f.endswith('.json')], key=lambda f: int(f.split('.')[0]))
